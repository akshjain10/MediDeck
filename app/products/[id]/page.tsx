...
const ProductDetail = () => {
  ...
  const handleShare = async (platform: string) => {
      ...
      // Get product image for sharing
      const imageUrl = `${window.location.origin}/images/products/${product.id}.webp`;
      
      let shareText = `Check out ${product.brandName} by ${product.company} - MRP ₹${product.mrp}\n\n${productUrl}`;
      
      // For WhatsApp, include the image
      if (platform === 'whatsapp') {
        shareText = `🏥 *${product.brandName}*\n\n` +
                   `🏢 Company: ${product.company}\n` +
                   `💰 MRP: ₹${product.mrp}\n` +
                   `🏷️ Category: ${product.category}\n`;
        
        if (product.packing) {
          shareText += `📋 Packing: ${product.packing}\n`;
        }
        if (product.salt) {
          shareText += `🧪 Salt: ${product.salt}\n`;
        }
      ...
      switch (platform) {
        ...
        case 'email':
          ...
          break;
        case 'pinterest':
          window.open(`https://pinterest.com/pin/create/button/?url=${encodeURIComponent(productUrl)}&media=${encodeURIComponent(imageUrl)}&description=${encodeURIComponent(shareText)}`, '_blank');
          break;
        case 'facebook':
